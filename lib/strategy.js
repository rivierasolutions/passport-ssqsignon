 ...
 * @api public
 */
function Strategy(options, module) {
  if (typeof options == 'string') {
    module = options;
    options = {};
  }
  if (!module) { throw new TypeError('This strategy requires a module name'); }
  
  passport.Strategy.call(this);
  this.name = 'tinyusers';
  this._verify = buildVerify(module);
...
function buildVerify(module) {
        ...
        return https.get({
            ...
            if (response.statusCode == 200) {
                response.on('data', function(body) {
                    ...
                    done(null, { id: parsed.user_id, scope: scopeAsObject(parsed.scope) });
                });
            } else {
    ...
    function scopeAsObject(scopeStr) {
        var scope = {};
        scopeStr.split(' ').map(function(s) { scope[s] = true; });
        return scope;
    }
}
